...
fun ClientFirebase.toEntity() = ClientEntity(
    id = id.toIntOrNull()
        ?: 0, // Assuming id from Firebase can be String, converting to Int for Entity
    name = name,
    email = email ?: "", // Handle null email from Firebase
    address = address ?: "", // Map Firebase address, default to empty if null
    paymentPreference = paymentPreference
        ?: "", // Map Firebase preference, default to empty if null
    phone = phone ?: ""
)

fun ClientFirebase.toDomain() = Client(
    id = id.toIntOrNull()
        ?: 0, // Assuming id from Firebase can be String, converting to Int for Domain
    name = name,
