    ...
    override fun getOrders(): Flow<List<Order>> =
        ...
        try {
            orderFirebaseDataSource.getOrders().collect { firebaseOrders ->
                ...
                val clients = clientDao.getAllClientsBlocking() // Blocking call for simplicity, consider Flow
                val products = productDao.getAllProductsBlocking() // Blocking call for simplicity, consider Flow
                emit(firebaseOrders.map { it.toDomain(clients, products) }) // Emit Firebase data mapped to domain
            }
        } catch (e: Exception) {
