...
import com.your_app_name.data.remote.firebase.datasource.PurchaseFirebaseDataSource
...
class PurchaseFirebaseDataSourceImpl @Inject constructor(
    ...
    override suspend fun getPurchaseById(purchaseId: String): PurchaseFirebase? {
        return try {
            purchasesCollection.document(purchaseId).get().await().toObject(PurchaseFirebase::class.java)
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
