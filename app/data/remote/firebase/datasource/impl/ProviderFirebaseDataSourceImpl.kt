...
class ProviderFirebaseDataSourceImpl @Inject constructor(
    ...
    override suspend fun getProviderById(providerId: String): ProviderFirebase? {
        return try {
            providersCollection.document(providerId).get().await().toObject(ProviderFirebase::class.java)
        } catch (e: Exception) {
            // Log or handle the error
            null
        }
    ...
    override suspend fun updateProvider(provider: ProviderFirebase) {
        try {
            providersCollection.document(provider.id).set(provider).await() // Firestore set updates or creates
        } catch (e: Exception) {
            // Log or handle the error
            throw e // Re-throw or handle as needed
        }
