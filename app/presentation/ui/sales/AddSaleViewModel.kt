...
class AddSaleViewModel @Inject constructor(
    ...
    fun addProductToSale(product: Product) {
        ...
        if (existingItem != null) {
            ...
            _selectedProducts.value = currentItems.map { if (it.productId == product.id) updatedItem else it }
        } else {
    ...
    fun updateProductQuantity(saleItem: SaleItem, quantity: Int) {
        ...
        val currentItems = _selectedProducts.value.toMutableList()
        val index = currentItems.indexOfFirst { it.id == saleItem.id } // Use unique ID if available, otherwise productId
    ...
    fun removeProductFromSale(saleItem: SaleItem) {
        _selectedProducts.value = _selectedProducts.value.filter { it.id != saleItem.id } // Use unique ID if available, otherwise productId
        calculateTotal()
    }
    ...
    fun saveSale() {
        viewModelScope.launch {
            ...
            when (val result = addSaleUseCase(newSale)) {
                ...
                is Resource.Error -> {
                    _savingState.value = SavingState.Error(result.message ?: "Unknown error saving sale")
                }
                is Resource.Loading -> {
